...
class UpsamplingStage(nn.Module):
    """Residual processing followed by DINO-WM's two 2x upsampling layers."""
...
class DinoPatchDecoder(nn.Module):
    """Decode ``(B,P,D)`` or ``(B,T,P,D)`` DINO patch tokens to RGB."""
    ...
    def __init__(
        ...
        if channels % 2:
            raise ValueError("channels must be even for transposed convolutions")
